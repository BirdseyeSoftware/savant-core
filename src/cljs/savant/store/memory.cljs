...
;;
;; src/clj/savant/store/memory.clj @ Thu Nov 08 22:49:23 UTC 2012
;;
...
(defrecord MemoryEventStream [state-atom current-rev] IEventStream (get-rev-id [this] current-rev) (get-tip [this] (-get-stream-tip (clojure.core/deref state-atom))) (tip? [this] (= current-rev (get-tip this))) (get-commits-seq [this] (let [commits (seq (clojure.core/deref state-atom))] (if (nil? current-rev) commits (take-while (fn* [p1__5449#] (not= current-rev (-> p1__5449# meta :event-store/parent-rev-hash))) commits)))) (get-events-seq [this] (seq (get-events-vec this))) (get-events-vec [this] (into [] (flatten (get-commits-seq this)))) (get-events-vec [this from-event-id] (subvec (get-events-vec this) from-event-id)) (get-events-vec [this from-event-id to-event-id] (subvec (get-events-vec this) from-event-id to-event-id)) (commit-events! [this events] (let [parent-hash (or current-rev (-build-first-commit-hash this)) new-rev-hash (hex-digest [parent-hash events])] (letfn [(update-stream-state [stream-state] (let [tip (-get-stream-tip stream-state)] (if (not (= current-rev tip)) (throw+ [:type :event-store/conflict-detected] (format "conflict - current: %s and tip: %s" current-rev tip)) (conj stream-state (with-meta-merge events {:event-store/parent-rev-hash parent-hash, :event-store/rev-hash new-rev-hash})))))] (swap! state-atom update-stream-state) (-bind-stream-to-rev this new-rev-hash)))))
